...
For starters, [Use this template](https://github.com/new?template_name=FabricModTemplate&template_owner=nobaboy).
...
This template uses [Stonecutter](https://stonecutter.kikugie.dev/stonecutter/launch/) for multi-version support and [DevAuth](https://github.com/DJtheRedstoner/DevAut/) for logging in with your Minecraft account (if desired, you can opt out of using DevAuth by removing it from `build.gradle.kts`).
...
When importing the project into IntelliJ, remember to set both Gradle JVM and Project SDK to use Java 21 JDK, then click on the sync button, it should create run tasks based on the versions you selected with Stonecutter.
If it doesn't then try restarting IntelliJ.<br>
You will get preconfigured launch configs based on the selected versions you set with [Stonecutter's Settings](https://github.com/nobaboy/FabricModTemplate/blob/master/settings.gradle.kts#L16-L24).

I highly suggest you head over to [Stonecutter's setup page](https://stonecutter.kikugie.dev/stonecutter/setup) to understand it better, or at the very least the [launch page](https://stonecutter.kikugie.dev/stonecutter/launch) to better know how to manage your mod versions, run minecraft, and export the mod, alternativly you can read this quick guide (provided you use IntelliJ IDEA):
